...
public class BaseContainer extends Container {
	...
	public static Image getAwesomeImage(char c, int height, int color)
	...
	public void initUI() {
		try {
			...
			setInsets(0, 0, headerBar.getHeight(),
					footerBar.getPreferredHeight());
			add(footerBar, LEFT, BOTTOM + insets.bottom, FILL, PREFERRED);
			// we use a PressListener so that the subclasses don't need to call
			// super.onEvent
				...
				public void controlPressed(ControlEvent e) {
					...
					try {
						...
						case 1: {
							...
							String s = "Paint 100x elapsed: " + (fim - ini)
									+ "ms";
							Toast.show(s, 3000);
						...
						case 2: {
							...
							if (isMainMenu) {
								...
								if (helpMessage == null)
									return;
								MessageBox mb = new MessageBox("Help",
										helpMessage, new String[] { "Close" });
			...
			setTitle(name.endsWith("Sample") ? name.substring(
					name.lastIndexOf('.') + 1, name.length() - 6)
					: defaultTitle);
		} catch (Exception ee) {
			MessageBox.showException(ee, true);
		}
	...
	public void back() {
		try {
			...
			// MainWindow.exit(0); // we're the last screen, so just exit
			// the application
		}
	}

	public boolean ask(String question) {
		MessageBox mb = new MessageBox("Question", question, new String[] {
				"Yes", "No" });
		mb.popup();
